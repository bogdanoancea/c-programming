...
void modificare(char* fn) {
	...
	printf("\nIntroduceti numarul matricol al studentului a carui medie doriti sa o modificati sau -1 pentru terminare:");
	scanf("%d", &nr_mat);
	while(nr_mat !=- 1) {
        do {
            n_art = fread(&s, sizeof(s), 1, f);
            if(s.nr_matricol == nr_mat) {
                pos = ftell(f) - sizeof(s);
                break;
            }
            printf("\ndebug nart %d", n_art);
        } while(s.nr_matricol != nr_mat && n_art == 1);
        if(n_art < 1) {
